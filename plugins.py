...
def mono(image, sepia=False):
    ...
    height = len(image)
    width = len(image[0])
    if not type(sepia) == bool:
        AssertionError("True or False only.")
    if not sepia:
        for row in range(height):
            for col in range(width):
                pixel = image[row][col]
                result = 0.3 * pixel.red + 0.6 * pixel.green + 0.1 * pixel.blue
                pixel.red = int(result)
                pixel.blue = int(result)
                pixel.green = int(result)
    if sepia:
        for row in range(height):
            for col in range(width):
...
def flip(image, vertical=False):
    ...
    if not type(vertical) == bool:
        AssertionError("True or False only.")
    if vertical:
        for x in range(height//2):
            for y in range(width):
                image[x][y], image[-x - 1][y] = image[-x -1][y], image[x][y]
    if not vertical:
        for x in range(height):
            for y in range(width//2):
                image[x][y], image[x][-y - 1] = image[x][-y - 1], image[x][y]
    return True
...
def transpose(image):
    ...
    # Change this to return True when the function is implemented
    numRows = len(image)
    numColms = len(image[0])
    result = []
    for rw in range(numColms):
        row = []
        for cl in range(numRows):
            row.append(image[cl][rw])
        result.append(row)
